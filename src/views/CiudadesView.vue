...
async function getCiudad() {
	const res = await axios.get('http://localhost:5000/ciudades');
	ciudades.value = res.data;
	console.log(res.data);
}
async function deleteCiudad(id) {
	try {
		await axios.delete(`http://localhost:5000/ciudades/${id}`);
		getCiudad();
	} catch (error) {
		console.log(error);
	}
...
async function updateCiudad(id) {
	try {
		await axios.put(`http://localhost:5000/ciudades/${id}`, {
			descripcionCiu: descUp.value,
			idciudad: idUp.value,
		});
		getCiudad();
	} catch (error) {
...
async function newId() {
	try {
		const newid = await axios.get('http://localhost:5000/idciudad');
...
async function newSexo() {
	if (ID.value && Desc.value) {
		error.value = false;
		try {
			await axios.post('http://localhost:5000/ciudades', {
				idciudad: ID.value,
				descripcionCiu: Desc.value,
			});
			guardado.value = true;
			getCiudad();
		} catch (error) {
...
<template>
	<section class="main__section">
		...
		<div class="lds-facebook" v-if="!ciudades.length">
			<div></div>
			<div></div>
			<div></div>
		</div>
		...
		<div class="data__table" v-else>
			...
			<div class="table__row" v-for="(ciudad, index) in ciudades" :key="index">
				<p>{{ ciudad.idciudad }}</p>

				<p>{{ ciudad.descripcionCiu }}</p>
				<button
					title="Eliminar"
					class="link__button row__button"
					@click="deleteCiudad(ciudad.idciudad)"
				>
				...
				<button
					title="Modificar"
					class="link__button row__button"
					@click="updateData(ciudad.idciudad, ciudad.descripcionCiu)"
				>
