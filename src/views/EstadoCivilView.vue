...
async function getCiudad() {
	const res = await axios.get('http://localhost:5000/ec');
	estadoCivil.value = res.data;
	console.log(res.data);
}
async function deleteCiudad(id) {
	try {
		await axios.delete(`http://localhost:5000/ec/${id}`);
		getCiudad();
	} catch (error) {
		console.log(error);
	}
...
async function updateCiudad(id) {
	try {
		await axios.put(`http://localhost:5000/ec/${id}`, {
			descripcionEc: descUp.value,
			idestado_civil: idUp.value,
		});
		getCiudad();
	} catch (error) {
		console.log(error);
...
async function newId() {
	try {
		const newid = await axios.get('http://localhost:5000/idestado_civil');
...
async function newSexo() {
	if (ID.value && Desc.value) {
		error.value = false;
		try {
			await axios.post('http://localhost:5000/ec', {
				idestado_civil: ID.value,
				descripcionEc: Desc.value,
			});
			guardado.value = true;
			getCiudad();
		} catch (error) {
...
<template>
	<section class="main__section">
        ...
        <div class="lds-facebook" v-if="!estadoCivil.length">
			<div></div>
			<div></div>
			<div></div>
		</div>
		<div class="data__table" v-else>
			...
			<div class="table__row" v-for="(ciudad, index) in estadoCivil" :key="index">
				<p>{{ ciudad.idestado_civil }}</p>

				<p>{{ ciudad.descripcionEc }}</p>
				<button
					title="Eliminar"
					class="link__button row__button"
					@click="deleteCiudad(ciudad.idestado_civil)"
				>
				...
				<button
					title="Modificar"
					class="link__button row__button"
					@click="updateData(ciudad.idestado_civil, ciudad.descripcionEc)"
				>
